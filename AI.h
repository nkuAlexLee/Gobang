...
struct _POINTS {//最佳落子位置,[0]分数最高,[19]分数最低
    QPoint pos[20];
    int score[20]={0};//此处落子的局势分数
};
struct DECISION {
    QPoint pos;//位置
    int eval=0;//对分数的评估
...
public://博弈树搜索部分
    ...
    _POINTS seekPoints(int board[15][15]);//生成对于白子的最佳20个落子位置及分数
    ...
    vector<QPoint> seek_kill_points(int board[15][15]);//找白棋的连5,活4,冲4,活3的杀棋位置
...
int chessAi::analyse(int(*board)[15], int depth, int alpha, int beta) {
    ...
    if (depth == 0 || EVAL.result != R_DRAW) {//抵达最深层/如果模拟落子可以分出输赢，那么直接返回结果，不需要再搜索
       ...
       //cout<<"白方决策！";
        ...
        for (int i = 0; i < 10; ++i) {
            //cout<<"白方模拟下"<<P.pos[i].x()<<","<<P.pos[i].y();
            int sameBoard[15][15];
       ...
       //cout<<"黑方决策！";
        ...
        _POINTS P = seekPoints(rBoard);//找对于黑子的最佳位置,需要将棋盘不同颜色反转,因为seekPoint是求白色方的最佳位置

        for (int i = 0; i < 10; ++i) {
            //cout<<"黑方模拟下"<<P.pos[i].x()<<","<<P.pos[i].y();
...
int chessAi::analysefirst(int(*board)[15], int depth, int alpha, int beta) {
    EVALUATION EVAL = evaluate(board);
    if (depth == 1 || EVAL.result != R_DRAW) {//抵达最深层/如果模拟落子可以分出输赢，那么直接返回结果，不需要再搜索
        nodeNum += 1;
        if (depth == 1) {
            _POINTS P;
            P = seekPoints(board);
    ...
    else if (depth % 2 == 1) {//max层,我方(白)决策
       //cout<<"白方决策！";
...
bool chessAi::analyse_kill(int(*board)[15], int depth) {
    ...
    if (depth == 0 || EVAL.result != R_DRAW) {
        ...
        if (depth == 16 || depth == 14) {//最开始4层选所有能走的10个点
            ...
            vector<QPoint> pointList = seek_kill_points(board);//产生杀棋点

            if (pointList.size() == 0)return false;//没有杀棋点
...
bool chessAi::analyse_kill_first(int(*board)[15], int depth) {
    ...
    if (depth == 1 || EVAL.result != R_DRAW) {
        if (depth == 1) {//若抵达最深层,走一步对白棋的最好位置,若白棋还没赢则返回false
            _POINTS P;
            P = seekPoints(board);
            board[P.pos[0].x()][P.pos[0].y()] = C_WHITE;
        ...
        if (depth == 17 || depth == 15) {//最开始4层选所有能走的10个点
            _POINTS P = seekPoints(board);
            for (int i = 0; i < 10; ++i) {
                int sameBoard[15][15];
                copyBoard(board, sameBoard);
...
vector<QPoint> chessAi::seek_kill_points(int(*board)[15]) {
    ...
    _POINTS P = seekPoints(board);//一般来说,能冲4或者活3的必在评分前20的点内
